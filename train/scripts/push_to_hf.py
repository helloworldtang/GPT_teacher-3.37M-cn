...
def push_to_hub(repo_id: str, checkpoint_path: str = "train/checkpoints/best.pt",
    ...
    # 模型权重
    files_to_upload.append(("train/checkpoints/best.pt", checkpoint_path))

    # 分词器
    if os.path.exists(tokenizer_path):
        files_to_upload.append(("train/tokenizer.json", tokenizer_path))

    # 配置
    if os.path.exists(config_path):
        files_to_upload.append(("train/config.yml", config_path))

    # 训练数据（用于重建分词器）
    for name in ["train.jsonl", "val.jsonl", "test.jsonl"]:
        path = os.path.join("train/data", name)
        if os.path.exists(path):
            files_to_upload.append((f"train/data/{name}", path))

    # 数据准备脚本
    prep_path = "train/data/prepare_data.py"
    if os.path.exists(prep_path):
        files_to_upload.append(("train/data/prepare_data.py", prep_path))
...
if __name__ == "__main__":
    ...
    push_p.add_argument("--ckpt", default="train/checkpoints/best.pt", help="模型路径")
